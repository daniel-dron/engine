...
vec3 calcPointLight(PointLight light, vec3 norm, vec3 dirToView) {
    ...
    // ambient
    vec3 ambient = light.ambient * intensity * texture(material.diffuse, fs_in.uvs).rgb;

    // diffuse
    float diff = max(dot(norm, lightDir), 0.0f);
    vec3 diffuse = light.diffuse * intensity * diff * texture(material.diffuse, fs_in.uvs).rgb;

    // specular
    vec3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(norm, halfwayDir), 0.0f), 16.0f);
    vec3 specular = spec * light.specular * intensity;
...
void main() {
    vec3 normals = texture(material.normal, fs_in.uvs).rgb;
    normals = normals * 2.0 - 1.0;
    vec3 norm = normalize(fs_in.TBN * normals);
